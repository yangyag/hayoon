...
어린이용 한글 학습 SPA. Vue 3.5(Composition API + `<script setup>`) + Vue Router 4 + Vite 기반의 단일 페이지 애플리케이션입니다. 백엔드에서 글자 목록과 단어를 받아오고, 브라우저 음성 합성(TTS)으로 단어/글자를 읽어주며, 학습 진행도를 `localStorage`에 저장합니다.

> React 18 → Vue 3.5 마이그레이션 결과를 반영. 백엔드 API·CORS·정적 에셋·배포 인프라는 변경 없음.
...
- 의존성
  ...
  - `vue-router` `^4.4.0`
- 개발 의존성
  - `vite` `^5.4.19`, `@vitejs/plugin-vue` `^5.1.0`
- npm 스크립트
  - `dev`: `vite`
  - `build`: `vite build`
...
## 2. 진입점 (`front/src/main.js`)과 루트 컴포넌트 (`front/src/App.vue`)

- `main.js`: `createApp(App).use(router).mount("#root")`로 마운트. 전역 스타일 `./styles.css` import.
- `App.vue`: `<div class="app-shell"><router-view/></div>` 래퍼. 라우터가 현재 경로에 해당 페이지를 렌더.

## 3. 라우팅 (`front/src/router/index.js`)

`createRouter({ history: createWebHistory(), routes })`(HTML5 history 모드). 경로는 다음과 같다.
...
| `/:pathMatch(.*)*` | `redirect: "/"` | 매칭되지 않는 경로는 홈으로 리다이렉트 |

화면 이동 흐름: `/` → `/library` → `/letters` → `/learn/:letterKey`

## 4. 페이지별 기능

모든 페이지는 Vue 3 `<script setup>`(Composition API) SFC.

### 4.1 WelcomePage (`front/src/pages/WelcomePage.vue`)

- 환영 문구("하윤이 환영해~")와 배경 장식 bg-orb 3개(orb-a, orb-b, orb-c) 표시
- "시작하기" 버튼 클릭 시 `router.push("/library")`

### 4.2 LibraryPage (`front/src/pages/LibraryPage.vue`)

- 상단 내비게이션에 "홈" 버튼(`router.push("/")`)
- "가나다 한글" 책 카드(`book-card`) 클릭 시 `router.push("/letters")`
- 현재 책은 하나만 하드코딩되어 있음("가나다 한글")

### 4.3 LettersPage (`front/src/pages/LettersPage.vue`)

글자 선택 그리드. 백엔드의 활성화 여부를 반영한다.

- 초기 상태: `letters` ref를 `LETTERS`(카탈로그 14개) 기준 전부 `enabled: false`로 세팅
- `onMounted`에서 `getLetters()` 호출하여 목록 갱신
  - 로딩 중 `loading` true → "글자를 불러오는 중..." 표시
  - 실패 시 `error`에 "글자 목록을 불러오지 못했어요." 설정하고 모든 글자를 `enabled: false`로 복원
  - `active` 플래그로 언마운트 후 상태 갱신 방지(`onUnmounted`에서 `active = false`, `noticeTimeoutId` 타이머 정리)
- 글자 버튼 클릭(`handleLetterClick`)
  - `letter.enabled === true`면 `` router.push(`/learn/${letter.key}`) ``로 이동
  - 비활성(잠금)이면 `showSoonMessage()`로 "곧 열려요" 안내를 1400ms 동안 표시(`noticeTimeoutId`로 타이머 관리)
- 버튼 클래스는 `:class="['letter-btn', enabled ? 'enabled' : 'locked']"`, `:aria-disabled="!letter.enabled"`
- 상단에 "뒤로"(→ `/library`), "홈"(→ `/`) 버튼

### 4.4 LearnPage (`front/src/pages/LearnPage.vue`)

핵심 학습 화면. URL의 `letterKey`로 해당 글자의 단어들을 불러와 셔플 순서로 한 장씩 보여준다.

- `useRoute().params.letterKey` 취득, 기본값 `""`
- `letterLabel`은 `computed(() => findLetterLabel(letterKey.value))`
- 주요 상태(`ref`): `words`, `loading`, `error`, `showIntro`, `imageFailed`, `cycle`, `ttsSupported`, `ttsNotice`
- `currentWord`는 `computed(() => cycle.value.current)`
...
- `drawNextWord()`: `order`가 비었거나 `cursor`가 끝에 도달하면 새로 셔플하고 `cursor=0`. 다음 단어 선택 후 `cursor+1`, `seenCount`는 `(seenCount % words.length) + 1`로 순환 카운트. `cycle.value`를 새 객체로 갱신.

#### 라이프사이클 / 부수효과

- `onMounted`: `isTtsSupported()` 결과를 `ttsSupported`와 `saveTtsSupport()`로 저장. `onUnmounted`: `stopSpeaking()` + 진행 중 로드 무효화(`loadToken` 증가)
- `watch(letterKey, …, { immediate: true })`: `letterKey` 변경마다 `readLetterProgress(letterKey)`로 저장된 진행도 읽고 `saveLastLetterKey(letterKey)` 기록 후 `getLetterWords(letterKey)` 호출
  - 로딩 중 '학습 카드를 준비하는 중...' 메시지 표시
  - 성공: `words.value = items` + `buildInitialCycle`로 사이클 초기화
  - 실패: `words=[]`, `error="단어를 불러오지 못했어요."`
  - 레이스 가드: `loadToken` 토큰으로 진행 중 비동기 로드 무효화(빠른 `letterKey` 변경/언마운트 시 최신 키 결과만 반영)
- `watch(() => cycle.value.current?.id, …)`: `imageFailed`를 `false`로 리셋
- `watch([cycle.current?.id, cycle.lastWordId, cycle.seenCount, error, letterKey, showIntro], …)`: 인트로가 아니고 현재 단어가 있고 에러가 없을 때 `saveLetterProgress(letterKey, { lastWordId, seenCount })`로 진행도 저장

#### 화면 구성

- 인트로 카드(`showIntro` true): "오늘의 글자" + 큰 글자 버튼. 버튼 클릭 시 `handleSpeakLetter()`로 글자 라벨을 TTS로 읽음
- 단어 카드(`showIntro` false):
  - 이미지(`currentWord.imageUrl`). 로드 실패 시 `@error`로 `imageFailed=true`가 되어 "이미지 준비 중" 플레이스홀더 표시
  - 단어 텍스트: 첫 글자만 `<strong>`으로 강조(`<template v-if="currentWord?.word">` 블록)
...
- `saveTtsSupport(ttsSupported)`: `ttsSupported`를 boolean으로 저장(LearnPage `onMounted` 시 호출)
...
- `/home/yangyag/hayoon/front/src/main.js`
- `/home/yangyag/hayoon/front/src/App.vue`
- `/home/yangyag/hayoon/front/src/router/index.js`
- `/home/yangyag/hayoon/front/src/api.js`
- `/home/yangyag/hayoon/front/src/pages/WelcomePage.vue`
- `/home/yangyag/hayoon/front/src/pages/LibraryPage.vue`
- `/home/yangyag/hayoon/front/src/pages/LettersPage.vue`
- `/home/yangyag/hayoon/front/src/pages/LearnPage.vue`
